...
const CartPage = () => {
  ...
  const isLogger = JSON.parse(localStorage.getItem("isLogger")) || false;
      ...
      render: (text, record) => (
        <div className="flex items-center">
          ...
          <div className="flex flex-col gap-2 ml-2 items-start">
            ...
            <span className="max-w-28">Size: {record?.DrinkSize?.Size}</span>
          </div>
        </div>
      ),
    },
        ...
        <InputNumber
          ...
          onChange={(value) => {
            const newData = cartInfo.map((item) => {
              if (
                item.Id === record.Id &&
                item?.DrinkSize?.Size === record?.DrinkSize?.Size
              ) {
                item.quantity = value;
            ...
            setCartInfo(newData);
            localStorage.setItem("cartInfo", JSON.stringify(newData));
          }}
        />
      ),
    },
      ...
      render: (text, record) => {
        if (
          ...
          if (check.length) {
            return (
              <Space>
                <span className="line-through">{formatVND(text)}</span>
                <span className="text-red-500 font-bold">
                  {formatVND(text - text * (applyVoucherCode?.Discount / 100))}
                </span>
              </Space>
            );
  ...
  const totalAmount = cartInfo?.reduce((sum, item) => {
    if (!applyVoucherCode && !voucherCode) {
      ...
      if (check?.length) {
        return (
          sum + item.total - (item.total * applyVoucherCode.Discount) / 100
        );
      }
      return sum + item.total;
    }
  ...
  return (
    <Elements stripe={stripePromise}>
      <div className="container max-w-[1200px] mx-auto p-4 my-8">
        ...
        <Table columns={columns} dataSource={cartInfo} pagination={false} />
        <div className="flex justify-between items-center mt-8">
          <div className="flex flex-col gap-2 items-start">
            ...
            <span className="text-xl">
              ...
              <span className="text-red-500">{formatVND(totalAmount)}</span>
            </span>
          </div>
          {role === "staff" ? (
            <Space>
            ...
            <Button
              ...
              disabled={!customerAddress.length || !cartInfo.length}
            >
              Thanh toán online
            </Button>
          )}
